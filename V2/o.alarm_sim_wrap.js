 ...
 *
 * вызывается из модуля o.alarm_main.js из функции DrawDiag()
 * 
 * замена старого метода initDynamicSHC()
 *
 * ********************************************************* */
...
//		alert(data.error);
	...
	var onServerData = function (subscriptionConfig, responseToSubscribe) {
		...
		// здесь только выкладываем данные в SHC структуру и оцениваем (opp_analog_inject или opp_digital_inject)
		// и квитируем параметры
		...
		for (let i in subscriptionConfig.var_list) {
			...
			let pt_name = varName.replace(/.o_OUT$/gi, "-OUT");
	...
	* */
	...
	var onConnectedToSimulator = function () {
		subscription = simServer.subscribe(simServerVarList, onServerData);
	...
	if (!debugValStat) {
		...
		//	} else {
		// !!! отладочная ветка !!!
		//		RandomValue( simServerVarList );	// Шумим значением параметров
...
/*
	Отписываемся в функции setUrlQueryArgs (o.alarm_main.js)
 */
};
 ...
 * fake
 *
 * все происходит в функции onServerData
 */
