	...
	{	// Анализ Текущих и Предыдущих параметров страницы
		...
		if (!deepEqual(oldQueryArgs, QueryArgs)) {
			...
			if (oldQueryArgs.diag != QueryArgs.diag) {
				...
				diagLoad = loadScript((window["itsRootUrl"] ? window["itsRootUrl"] : '')+'api/mmi/' + projectName + '/ovation/diag/' + QueryArgs.diag + '.min.diag');
//				diagLoad = loadScript('./diag/' + QueryArgs.diag + '.min.diag');
				diagLoad.then(function (result) {
					...
					setPDSPoints();
					// Подписываемся на получение динамических данных от 3KeyMaster
			...
			if ((oldQueryArgs.group != QueryArgs.group) && (QueryArgs.group != 0)) {
				...
				groupLoad = loadScript((window["itsRootUrl"] ? window["itsRootUrl"] : '') +'api/mmi/' + projectName + '/ovation/group/' + QueryArgs.group + '.group');
