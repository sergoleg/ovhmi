...
test('throws with invalid constructor', () => {
  delete global.WebSocket;
  expect(() => {
    new _reconnectingWebsocket.default(URL, undefined, {
      WebSocket: 123,
      maxRetries: 0
    });
  }).toThrow();
...
test('throws if not created with `new`', () => {
  expect(() => {
    // @ts-ignore
    (0, _reconnectingWebsocket.default)(URL, undefined);
  }).toThrow(TypeError);
});
test('global WebSocket is used if available', done => {
  // @ts-ignore
  const ws = new _reconnectingWebsocket.default(URL, undefined, {
    maxRetries: 0
  });

  ws.onerror = () => {
    // @ts-ignore
    expect(ws._ws instanceof _ws.default).toBe(true);
    done();
  };
});
...
test('URL provider', async () => {
  ...
  expect(await ws._getNextUrl(url)).toBe(url); // @ts-ignore - accessing private property

  expect(await ws._getNextUrl(() => url)).toBe(url); // @ts-ignore - accessing private property

  expect(await ws._getNextUrl(() => Promise.resolve(url))).toBe(url); // @ts-ignore - accessing private property

  expect(() => ws._getNextUrl(123)).toThrow(); // @ts-ignore - accessing private property
...
test('websocket protocol', done => {
  ...
  const ws = new _reconnectingWebsocket.default(URL, anyProtocol);
...
test('null websocket protocol', done => {
  ...
  const ws = new _reconnectingWebsocket.default(URL, null, {});
  ws.addEventListener('open', () => {
    expect(ws.url).toBe(URL);
    expect(ws.protocol).toBe('');
    ws.close();
...
test('connection status constants', () => {
  ...
  expect(_reconnectingWebsocket.default.CONNECTING).toBe(0);
  expect(_reconnectingWebsocket.default.OPEN).toBe(1);
  expect(_reconnectingWebsocket.default.CLOSING).toBe(2);
  expect(_reconnectingWebsocket.default.CLOSED).toBe(3);
...
const maxRetriesTest = (count, done) => {
  const ws = new _reconnectingWebsocket.default(URL, undefined, {
    maxRetries: count,
...
test('level2 event listeners', done => {
  ...
  ws.addEventListener('open', fail);
  ws.removeEventListener('open', fail); // @ts-ignore
...
test('level2 event listeners using object with handleEvent', done => {
  ...
  ws.addEventListener('unknown1', fail); // @ts-ignore

  ws.addEventListener('open', fail); // @ts-ignore

  ws.addEventListener('open', fail); // @ts-ignore

  ws.removeEventListener('open', fail); // @ts-ignore

  ws.removeEventListener('unknown2', fail); // @ts-ignore
...
test('connection timeout', done => {
  const proc = (0, _child_process.spawn)('node', [`${__dirname}/unresponsive-server.js`, PORT_UNRESPONSIVE, '5000']);
  let lock = false;
  proc.stdout.on('data', () => {
    if (lock) return;
    lock = true;
    const ws = new _reconnectingWebsocket.default(`ws://localhost:${PORT_UNRESPONSIVE}`, undefined, {
...
test('binaryType', done => {
  ...
  ws.addEventListener('open', () => {
    expect(ws.binaryType).toBe('arraybuffer'); // @ts-ignore
...
test('immediately-failed connection should not timeout', done => {
  const ws = new _reconnectingWebsocket.default('ws://255.255.255.255', undefined, {
    maxRetries: 2,
    connectionTimeout: 500
  });
...
test('immediately-failed connection with 0 maxRetries must not retry', done => {
  const ws = new _reconnectingWebsocket.default('ws://255.255.255.255', [], {
    maxRetries: 0,
...
test('enqueue messages before websocket initialization with expected order', done => {
  ...
  const ws = new _reconnectingWebsocket.default(URL);
  const messages = ['message1', 'message2', 'message3'];
  messages.forEach(m => ws.send(m)); // @ts-ignore - accessing private field
...
test('closing from the other side should reconnect', done => {
  ...
  wss.on('connection', client => {
    ...
    if (i === max) {// will close from client side
    }
  ...
  ws.addEventListener('open', () => {
    ...
    if (j === max) {
      ws.close(); // wait a little to ensure no new connections are opened
...
test('closing from the other side should allow to keep closed', done => {
  ...
  ws.addEventListener('close', e => {
    if (e.code === codes[0]) {// do nothing, will reconnect
    }
...
test('reconnection delay grow factor', done => {
  const ws = new _reconnectingWebsocket.default('wss://255.255.255.255', [], {
    minReconnectionDelay: 100,
...
test('minUptime', done => {
  ...
  ws.addEventListener('close', () => {
    if (closeCount < expectedDelays.length) {
      // @ts-ignore - accessing private field
      expect(ws._getNextDelay()).toBe(expectedDelays[closeCount]); // @ts-ignore - accessing private field
...
test('reconnect after closing', done => {
	...
	ws.addEventListener('close', () => {
		if (i === 1) setTimeout(() => { ws.reconnect(); }, 1000);

		if (i === 2) {
			wss.close(() => { setTimeout(() => { done(); }, 1000); });
		}

		if (i > 2) { throw Error('no more expected reconnections'); }
	});
});
